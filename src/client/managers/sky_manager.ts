      ...
      if (elapsedMs >= this.colorEffect.duration) {
        ...
        if (material instanceof BABYLON.StandardMaterial) {
          ...
          const r = Math.max(
            ...
            Math.min(
              1,
              this.colorEffect.originalEmissiveColor.r + this.colorEffect.color.r * blendFactor
            )
          );
          const g = Math.max(
            0,
            Math.min(
              1,
              this.colorEffect.originalEmissiveColor.g + this.colorEffect.color.g * blendFactor
            )
          );
          const b = Math.max(
            0,
            Math.min(
              1,
              this.colorEffect.originalEmissiveColor.b + this.colorEffect.color.b * blendFactor
            )
          );
